...
#[derive(Clone, Debug)]
pub struct ShadowMappingUniforms {
    lights: Vec<Light>,
    shadow_bias_minimum: f32,
    shadow_bias_factor: f32,
    shadow_blur_half_kernel_size: i32,
...
impl Default for ShadowMappingUniforms {
    fn default() -> Self {
        Self {
            lights: vec![],
...
impl ShadowMappingPass {
    pub fn new(state: &mut RenderState, display: &Display) -> Self {
        ...
        let shadow_mapping_uniform =
            UniformBuffer::new(display.device(), ShadowMappingUniforms::default());
    ...
    pub fn run(
        ...
        self.shadow_mapping_uniform.update(
            display.queue(),
